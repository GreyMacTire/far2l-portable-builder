...
apt-get install libwxgtk3.0-gtk3-dev libx11-dev libxi-dev libxml2-dev libuchardet-dev cmake pkg-config g++ git patchelf makeself dpkg-dev libtree

mkdir -p $BUILD_DIR

cmake -S $REPO_DIR -B $BUILD_DIR -DUSEWX=yes -DARCLITE=no -DNETROCKS=no -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release
